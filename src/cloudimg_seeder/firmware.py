...
@functools.cache
def _firmware_search_dirs_cached(binary: str | None) -> tuple[Path, ...]:
    dirs: list[Path] = []
    seen: set[Path] = set()
    ...
    if resolved_binary is None:
        try:
            resolved_binary = find_qemu_binary("qemu-system-aarch64")
        except QemuError:
            resolved_binary = None

    if resolved_binary is not None:
        for d in _dirs_from_binary(resolved_binary):
            add(d)
        add(_print_datadir(resolved_binary))
...
def firmware_search_dirs(binary: str | None = None) -> list[Path]:
    ...
    Results are cached per ``binary`` value for the process lifetime, since
    each candidate involves a subprocess call.
    """
    return list(_firmware_search_dirs_cached(binary))
...
def _write_empty_vars(dest: Path) -> None:
    """Write a 64 MiB varstore filled with 0xFF, the erased-flash state."""
    chunk = b"\xff" * _EMPTY_VARS_CHUNK
    with dest.open("wb") as f:
        for _ in range(_EMPTY_VARS_BYTES // _EMPTY_VARS_CHUNK):
            f.write(chunk)


def prepare_edk2_aarch64_vars(dest: Path, code: Path | None = None) -> Path:
    """Copy the EDK2 vars template to dest, or create an empty erased store."""
    vars_src = find_edk2_aarch64_vars(code)
    if vars_src is not None:
        shutil.copy(vars_src, dest)
    else:
        _write_empty_vars(dest)
    return dest
